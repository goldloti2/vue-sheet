...
表單版的 `DataDetail`：依 `column.type` 自動選輸入元件（`text` → `v-text-field`、`number` → `v-number-input`、`date` → `v-date-input`、`select` → `v-select`、`ref` → `v-autocomplete`，對方整表的可搜尋下拉清單，每列文字是對方的 `$label`；`image` 跟 `text` 一樣是 `v-text-field`，貼網址或 Drive 連結），依 `schema.formOrder` 排序。新增與編輯共用，差別只在初始值。
